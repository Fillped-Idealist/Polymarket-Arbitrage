...
/**
 * 测试 Reversal V8.6 策略（优化版）
 */

const createV86Config = () => ({
...
async function main() {
  ...
  console.log('Reversal V8.6 全数据集交叉验证（优化版）');
  console.log('='.repeat(80));
  console.log(`\n测试数据集数量: ${datasets.length}`);
  console.log(`策略版本: Reversal V8.6\n`);
  ...
  const outputPath = path.join(__dirname, '../scripts/v86-full-dataset-results.json');

  fs.writeFileSync(outputPath, JSON.stringify({
    timestamp: new Date().toISOString(),
    strategy: 'Reversal V8.6',
