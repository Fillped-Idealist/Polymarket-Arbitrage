...
/**
 * 策略优化脚本 V5.0
 * 使用真实历史数据进行50轮优化，目标月利润率>50%
 *
 * V5.0 改进：
 * 1. 添加随机扰动，避免陷入局部最优
 * 2. 扩大参数调整范围
 * 3. 增加迭代次数到 50 轮
 * 4. 添加早停机制（连续 10 轮无改善则停止）
 */
...
const OUTPUT_FILE = path.join(__dirname, '../optimization_v5_results.json');
const LOG_FILE = '/tmp/optimization_v5.log';
...
class V5ParameterOptimizer {
  constructor(initialParams) {
    ...
    this.maxNoImprovement = 10; // 连续10轮无改善则停止
  }
  ...
  addRandomPerturbation() {
    ...
    // 随机扰动部分参数（30%概率）
    if (Math.random() < 0.3) {
      this.params.reversal.minPrice = perturb(this.params.reversal.minPrice, 0.02);
      console.log(`   → [随机] 扰动 Reversal minPrice → ${(this.params.reversal.minPrice * 100).toFixed(0)}%`);
    }

    if (Math.random() < 0.3) {
      this.params.reversal.maxPrice = perturb(this.params.reversal.maxPrice, 0.05);
      console.log(`   → [随机] 扰动 Reversal maxPrice → ${(this.params.reversal.maxPrice * 100).toFixed(0)}%`);
    }

    if (Math.random() < 0.3) {
      this.params.convergence.minPrice = perturb(this.params.convergence.minPrice, 0.03);
      console.log(`   → [随机] 扰动 Convergence minPrice → ${(this.params.convergence.minPrice * 100).toFixed(0)}%`);
    }
  }

  // V5.0：基于多学科视角的智能参数调整（扩大参数范围）
  adjustParameters(result) {
    ...
    console.log(`   无改善轮次: ${this.noImprovementCount}/${this.maxNoImprovement}`);

    const adjustments = [];

    // 【量化工程师视角】盈亏比优化（V5.0: 扩大调整范围）
    if (avgMonthlyReturn < 0) {
      console.log('   → [量化] 亏损状态，优化盈亏比');

      if (avgWinRate < 0.3) {
        // 胜率低，提高信号质量
        this.params.reversal.minPrice = Math.max(0.01, this.params.reversal.minPrice + 0.02);
        this.params.reversal.maxPrice = Math.min(0.70, this.params.reversal.maxPrice - 0.03);
        adjustments.push(`提高Reversal价格区间 → ${(this.params.reversal.minPrice * 100).toFixed(0)}%-${(this.params.reversal.maxPrice * 100).toFixed(0)}%`);
      }

      if (avgTrades < 20) {
        // 交易少，降低止损，增加机会
        this.params.reversal.stopLoss = Math.max(0.20, this.params.reversal.stopLoss - 0.08);
        this.params.convergence.stopLoss = Math.max(0.05, this.params.convergence.stopLoss - 0.03);
    ...
    // 【金融研究员视角】概率调整（V5.0: 扩大调整范围）
    if (avgWinRate < 0.35 && avgMaxDrawdown < 0.3) {
      console.log('   → [金融] 增加持仓捕捉机会');
      this.params.reversal.maxPositions = Math.min(20, this.params.reversal.maxPositions + 2);
      this.params.convergence.maxPositions = Math.min(15, this.params.convergence.maxPositions + 2);
      ...
      this.params.reversal.maxPositions = Math.max(3, this.params.reversal.maxPositions - 2);
      this.params.convergence.maxPositions = Math.max(5, this.params.convergence.maxPositions - 2);
      adjustments.push(`减少持仓 → R:${this.params.reversal.maxPositions} C:${this.params.convergence.maxPositions}`);
    }

    // 【数学家视角】期望收益优化（V5.0: 扩大调整范围）
    ...
    // 【统计学家视角】波动性管理（V5.0: 扩大调整范围）
    if (avgTrades > 50 && avgWinRate < 0.4) {
      console.log('   → [统计] 过度交易，提高门槛');
      this.params.reversal.minPrice = Math.min(0.10, this.params.reversal.minPrice + 0.02);
      this.params.convergence.minPrice = Math.min(0.85, this.params.convergence.minPrice + 0.03);
  ...
  // V5.0：检查是否应该早停
  shouldStopEarly() {
    return this.noImprovementCount >= this.maxNoImprovement && this.iteration > 10;
  }

  validateParams() {
    // Reversal参数验证（V5.0: 扩大范围）
    this.params.reversal.maxPositions = Math.max(3, Math.min(25, this.params.reversal.maxPositions));
    this.params.reversal.stopLoss = Math.max(0.15, Math.min(0.6, this.params.reversal.stopLoss));
    this.params.reversal.takeProfit = 1.0;
    this.params.reversal.minPrice = Math.max(0.01, Math.min(0.2, this.params.reversal.minPrice));
    this.params.reversal.maxPrice = Math.max(0.3, Math.min(0.75, this.params.reversal.maxPrice));

    // Convergence参数验证（V5.0: 扩大范围）
    this.params.convergence.maxPositions = Math.max(5, Math.min(35, this.params.convergence.maxPositions));
    this.params.convergence.stopLoss = Math.max(0.03, Math.min(0.25, this.params.convergence.stopLoss));
    this.params.convergence.takeProfit = 1.0;
    this.params.convergence.minPrice = Math.max(0.65, Math.min(0.85, this.params.convergence.minPrice));
    this.params.convergence.maxPrice = Math.max(0.9, Math.min(0.995, this.params.convergence.maxPrice));
...
const initialParams = {
  reversal: {
    maxPositions: 10,
    stopLoss: 0.25,  // V5.0: 从0.30降低，减少亏损
    takeProfit: 1.0,  // 分阶段止盈（0.85→0.95→0.99）
    minPrice: 0.05,  // V5.0: 从0.01开始，捕捉极低价格
    maxPrice: 0.65,  // V5.0: 扩展到65%
  },
  convergence: {
    maxPositions: 10,
    stopLoss: 0.08,  // V5.0: 从0.10降低，减少亏损
    takeProfit: 1.0,
    minPrice: 0.75,  // V5.0: 从0.80降低，扩大区间
    maxPrice: 0.995, // V5.0: 提高到99.5%
  },
  trend: { enabled: false },
  mean: { enabled: false },
...
const optimizer = new V5ParameterOptimizer(initialParams);
const results = [];
...
async function main() {
  ...
  for (let i = 1; i <= MAX_ITERATIONS; i++) {
    ...
    try {
      ...
      // 检查是否应该早停
      if (optimizer.shouldStopEarly()) {
        console.log(`\n⏹️  触发早停机制：连续 ${optimizer.noImprovementCount} 轮无改善`);
        console.log(`   最佳月利润率: ${optimizer.bestReturn.toFixed(2)}%`);
        break;
      }
      ...
      const remainingRounds = Math.min(MAX_ITERATIONS - i, optimizer.maxNoImprovement - optimizer.noImprovementCount);
...
function saveResults(currentRound) {
  const output = {
    ...
    dataFile: DATA_FILE,
    strategyVersion: 'V5.0-Enhanced',
    mode: 'enhanced-optimization-with-perturbation',
    currentRound,
